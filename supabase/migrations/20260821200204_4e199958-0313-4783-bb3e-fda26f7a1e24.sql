...
BEGIN
  ...
  IF o.id IS NOT NULL THEN
    ...
    IF eligible AND o.status IN ('completed','refilling') THEN
      BEGIN
        ms := (o.meta->>'completed_at')::bigint;
      EXCEPTION WHEN OTHERS THEN
        ms := NULL;
      END;
      started := CASE WHEN ms IS NOT NULL THEN to_timestamp(ms / 1000.0) ELSE COALESCE(_fallback_completed_at, o.updated_at) END;
...
BEGIN
  ...
  IF o.status NOT IN ('completed','refilling') THEN RAISE EXCEPTION 'Order is not completed yet'; END IF;
