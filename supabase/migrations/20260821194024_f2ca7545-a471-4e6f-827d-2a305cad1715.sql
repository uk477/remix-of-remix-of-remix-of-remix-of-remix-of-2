...
ALTER TYPE public.order_status ADD VALUE IF NOT EXISTS 'refilling';
...
ALTER TABLE public.order_refills
  ...
  ADD COLUMN IF NOT EXISTS refill_number integer;

CREATE INDEX IF NOT EXISTS order_refills_order_key_idx ON public.order_refills(order_key);

-- Customer-facing state: counts ONLY customer refills.
...
BEGIN
  ...
  VALUES (uid, _order_key, o.id, o.meta->>'order_ref', _client_token, 'requested', 'customer', o.status::text, used + 1)
...
BEGIN
  ...
  IF _status = 'completed' THEN
    new_meta := new_meta || jsonb_build_object('completed_at', (extract(epoch from now()) * 1000)::bigint);
  ELSE
    new_meta := new_meta - 'completed_at';
  END IF;
  ...
  VALUES (caller, 'order_status_override', 'order', _order_id::text,
          jsonb_build_object('from', prev, 'to', _status));
...
BEGIN
  ...
  SELECT count(*) + 1 INTO n FROM public.order_refills WHERE order_key = _order_id::text;
  ...
  INSERT INTO public.order_refills(
    ...
    o.user_id, _order_id::text, o.id, o.meta->>'order_ref', 'admin:' || gen_random_uuid()::text,
...
BEGIN
  ...
  FROM (
    ...
    WHERE r.order_key = _order_id::text
  ) s;

  SELECT count(*), max(requested_at) INTO cust_used, last_at
    FROM public.order_refills
   WHERE order_key = _order_id::text AND source = 'customer';
